...
namespace Agent
{
    internal class AgentFile
    {
        ...
        public bool HasTrayIcon => _charInfo.SystemTrayIcon;
        public Color TransparencyColor => _charInfo.TransparentColor;
        public Icon TrayIcon { get; private set; }

        private const uint _magic = 0xABCDABC3;

        public readonly CharacterInfo _charInfo;
        ...
        public AgentFile(string path)
        {
            if (!File.Exists(path))
                throw new FileNotFoundException("O caminho especificado não pôde ser encontrado.");

            _stream = new BinaryReader(new MemoryStream(File.ReadAllBytes(path)));

            if (_magic != _stream.ReadUInt32())
                throw new FormatException($"A assinatura não bate. Esperado: 0xABCDABC3");

            TrayIcon = SystemIcons.Error;

            ACSLocator charInfoPos = ReadLocator();
            _animInfo = ReadLocator();
            _imgInfo = ReadLocator();
            _audioInfo = ReadLocator();

            _charInfo = ReadCharInfo(charInfoPos);
            LoadAnimations();
        }
        ...
        private CharacterInfo ReadCharInfo(ACSLocator pos)
        {
            ...
            output.SystemTrayIcon = _stream.ReadBoolean();

            if (output.SystemTrayIcon) // ler o trayicon B-)
        ...
        public Bitmap ReadImage(uint index)
        {
            ...
            Parallel.For(0, imageData.Length, i => Marshal.Copy(imageData, i, bmpData.Scan0 + i, 1)); //Marshal.Copy(imageData, 0, ptrToPixel, imageData.Length);

            output.UnlockBits(bmpData);

            output.RotateFlip(RotateFlipType.RotateNoneFlipY);

            output.Palette = _charInfo.Palette;
            //---------------------
            return output;
        }

        public MemoryStream ReadAudioData(int index)
        {
